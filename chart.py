...
def create_radar_chart(km: float, cr: float, aw: float, sc: float) -> go.Figure:
    """4축 레이더 차트"""
    categories = ["대표자<br>리스크", "법인 경영<br>리스크", "리스크<br>인식", "시나리오"]
    values = [km, cr, aw, sc]
...
def create_bar_chart(km: float, cr: float, aw: float, sc: float) -> go.Figure:
    """카테고리별 수평 바 차트"""
    categories = ["시나리오", "리스크 인식", "법인 경영", "대표자"]
    values = [sc, aw, cr, km]
...
def render_consultant_tab():
    ...
    <div style="background: linear-gradient(135deg, #141e2b, #1a2736); padding: 18px; 
        ...
        <div style="font-size: 13px; color: #cbd5e1; line-height: 1.8;">
            ...
            리스크 인식({scores["aw_pct"]:.0f}%), 시나리오({scores["sc_pct"]:.0f}%)를 반영했습니다.
        </div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    <div style="background: rgba(0, 212, 170, 0.08); padding: 16px; border-radius: 8px; 
        ...
        <div style="font-size: 13px; color: #cbd5e1; line-height: 1.8;">
            저는 단순 보험 판매가 아닌, <b style="color: #00d4aa;">데이터 기반 시뮬레이션 도구</b>를 
            활용하여 "지금 이 상품에 가입하면 10년 후 얼마나 절세되는가"를 
            실시간으로 보여드립니다.<br><br>
            XAI, IoT, Streamlit 등 최신 기술로 <b>고객이 스스로 납득</b>할 수 있는 
            컨설팅을 제공합니다.
        </div>
    </div>
    ''', unsafe_allow_html=True)
