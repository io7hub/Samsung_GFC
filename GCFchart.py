"""
Kⁱ⁰⁷ 삼성생명 GFC | 법인 리스크 사전 진단표 v2.0
Streamlit 기반 AI 진단 엔진 + KPI 대시보드 + AI 컨설턴트

업그레이드 내역:
- AI 컨설턴트 탭 추가 (Ken 아바타 기반 상세 분석)
- 개선된 Plotly 차트 (레이더 차트 텍스트 잘림 해결)
- 다크 테마 UI 통일
- 종합 리스크율 가중평균 계산 개선

설치: pip install streamlit plotly numpy pillow
실행: streamlit run gfc_diagnosis_upgraded.py
"""
...
/* AI 컨설턴트 카드 */
.consultant-card { 
    background: linear-gradient(135deg, #0d2818, #1a3a2e); 
    padding: 16px; 
    border-radius: 10px; 
...
RESP_SCORE = {"예": 1.0, "일부 해당": 0.5, "아니오": 0.0}
...
AWARENESS = [
    ...
    ("단일 사고 발생 시 회사가 버틸 수 있는 기간을 알고 있다", 
     ["예", "아니오"], 
     {"예": 0.0, "아니오": 1.0}, 4),
...
# ═══════════════════════════════════════════════════════════
# PLOTLY CHARTS (Dark Theme + 개선된 레이더)
# ═══════════════════════════════════════════════════════════
PLOT_LAYOUT = dict(
...
def fig_gauge(pct):
    ...
    layout_config.update({
        'height': 235,
        'margin': dict(l=40, r=40, t=10, b=10)  # 좌우 여백 증가
    })
    fig.update_layout(**layout_config)
    fig.add_annotation(text=f"<b>{lbl}</b>", x=0.5, y=-0.01,
                       xref="paper", yref="paper", showarrow=False,
                       font=dict(size=14, color=clr, family="Noto Sans KR"))
    return fig

def fig_radar(km, cr, aw, sc):
    """4축 레이더 – 리스크율 (텍스트 잘림 해결)"""
    ...
    # PLOT_LAYOUT 복사 후 레이더 차트용으로 margin 수정
    layout_config = {**PLOT_LAYOUT}
    layout_config.update({
        'polar': dict(
            ...
            angularaxis=dict(
                ...
                rotation=0  # 텍스트 자동 회전 방지
            ),
            bgcolor="rgba(0,0,0,0)"
        ),
        'height': 290,  # 높이 증가
        'margin': dict(l=70, r=80, t=20, b=20),  # 여백 증가
        'showlegend': False
    })
    fig.update_layout(**layout_config)
    return fig
...
def build_script(info, scores, priority_items, sol_list):
    ...
    co = info.get("company", "WOORI(주)") or "WOORI(주)"
...
# AI CONSULTANT TAB
# ═══════════════════════════════════════════════════════════
def tab_consultant(info, scores, priority_items, sol_shown):
    """AI 컨설턴트 탭 - Ken이 대시보드 분석 설명"""
    total_pct = scores["total_pct"]
    lbl, clr = risk_level(total_pct)
    
    st.markdown('''
    <div style="font-family: 'Noto Sans KR'; font-size: 26px; font-weight: 700; color: #00d4aa; margin-bottom: 8px;">
        🤝 AI 컨설턴트 Ken의 종합 분석
    </div>
    ...
    # 컨설턴트 프로필
    st.markdown('''
    <div style="background: #111d2e; border: 1px solid #1e3a5f; border-radius: 10px; padding: 18px; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; gap: 16px;">
            <!-- 아바타 -->
            <div style="flex-shrink: 0;">
                <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #00d4aa, #00b894); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 36px; color: #fff; border: 3px solid #00d4aa; box-shadow: 0 4px 12px rgba(0,212,170,0.3);">
                    👨‍💼
                </div>
            </div>
            <!-- 텍스트 콘텐츠 -->
            <div style="flex: 1;">
                <div style="font-family: 'Noto Sans KR'; font-size: 16px; font-weight: 700; color: #00d4aa; margin-bottom: 8px;">
                    👋 안녕하세요, GFC 컨설턴트 Ken입니다
                </div>
                <div style="font-size: 12px; color: #94a3b8; line-height: 1.7; margin-bottom: 10px;">
                    20년간 한국-중남미 협력 프로젝트를 수행하며 <b>58개 사이트</b>의 디지털 전환(DX) 전략을 수립한 경험을 바탕으로, 귀사의 <b>종합 리스크 진단 결과</b>를 데이터 기반으로 상세히 분석해 드리겠습니다.
                </div>
                <div style="font-size:13px; color:#4a6a84;">
                📊 현재 분석 대상: <b style="color:#e0e6ed;"> {info.get('company','○○(주)') or '○○(주)'}</b> | {info.get('industry','') or '미입력'} · 종업원 {data["employees"]}명 · 연매출 {rev}억원
            </div>
            </div>
        </div>
    </div>
    ''', unsafe_allow_html=True)
    # 종합 진단 요약
    st.markdown(f'<div style="background: linear-gradient(135deg, #141e2b, #1a2736); padding: 18px; border-radius: 10px; border-left: 4px solid {clr}; margin-bottom: 16px;"><div style="font-size: 15px; font-weight: 700; color: {clr}; margin-bottom: 10px;">📋 종합 진단 결과</div><div style="font-size: 13px; color: #cbd5e1; line-height: 1.8;">귀사의 <b>종합 리스크율은 {total_pct:.0f}%</b>로 <b style="color: {clr};">{lbl}</b> 수준입니다.<br><br>4가지 핵심 리스크 카테고리를 가중평균하여 산출한 결과이며,<br>대표자 리스크({scores["km_pct"]:.0f}%), 법인 경영 리스크({scores["cr_pct"]:.0f}%), 리스크 인식({scores["aw_pct"]:.0f}%), 시나리오({scores["sc_pct"]:.0f}%)를 반영했습니다.</div></div>', unsafe_allow_html=True)
    
    # 우선 대응 항목
    st.markdown('<div style="font-size: 14px; font-weight: 700; color: #fff; margin: 16px 0 8px;">🔥 우선 대응 항목</div>', unsafe_allow_html=True)
    if not priority_items:
        st.markdown('<div class="gfc-empty">모든 항목 양호 🎉</div>', unsafe_allow_html=True)
    else:
        for i, it in enumerate(priority_items[:5]):
            wc = weight_color(it["w"])
            resp = "예" if it["score"] == 1.0 else "일부 해당"
            st.markdown(f'<div class="gfc-pri"><span class="rk" style="color: {wc};">#{i+1}</span><div class="info"><div class="cat">{it["section"]}</div><div class="txt">{it["text"]}</div></div><span class="wtag" style="background: {wc};">가중치 {it["w"]}</span><span class="wtag" style="background: #475569;">{resp}</span></div>', unsafe_allow_html=True)
    
    # Ken의 종합 컨설팅 제안
    st.markdown('<div style="font-size: 14px; font-weight: 700; color: #fff; margin: 20px 0 8px;">🎯 Ken의 종합 컨설팅 제안</div>', unsafe_allow_html=True)
    
    # 단기
    st.markdown('<div style="background: linear-gradient(135deg, #0d2818, #1a3a2e); padding: 14px; border-radius: 8px; border-left: 4px solid #00d4aa; margin-bottom: 10px;"><div style="font-size: 13px; font-weight: 700; color: #00d4aa; margin-bottom: 8px;">⏱️ 1. 단기 (3개월 이내)</div><div style="font-size: 12px; color: #c8d8e4; line-height: 1.7;">• 키맨 보험 가입으로 대표자 유고 리스크 대비<br>• 기업재해 보장 상품으로 종업원 안전망 구축<br>• 퇴직연금 제도 점검 및 최적화</div></div>', unsafe_allow_html=True)
    
    # 중기
    st.markdown('<div style="background: linear-gradient(135deg, #2a1f0d, #3a2f1d); padding: 14px; border-radius: 8px; border-left: 4px solid #f5a623; margin-bottom: 10px;"><div style="font-size: 13px; font-weight: 700; color: #f5a623; margin-bottom: 8px;">📅 2. 중기 (6~12개월)</div><div style="font-size: 12px; color: #c8d8e4; line-height: 1.7;">• 가업승계 시뮬레이션 (증여 vs 상속 세금 비교)<br>• 법인세 절세 전략 수립 (삼성생명 세무사 협업)<br>• 정기 재무 건강검진 체계 구축</div></div>', unsafe_allow_html=True)
    
    # 장기
    st.markdown('<div style="background: linear-gradient(135deg, #1a1a2e, #2a2a4e); padding: 14px; border-radius: 8px; border-left: 4px solid #6c5ce7; margin-bottom: 10px;"><div style="font-size: 13px; font-weight: 700; color: #a29bfe; margin-bottom: 8px;">🎯 3. 장기 (1~3년)</div><div style="font-size: 12px; color: #c8d8e4; line-height: 1.7;">• 후계자 육성 프로그램 및 지분 이전 계획<br>• 가족신탁, 재산분할 등 고급 절세 전략<br>• 삼성패밀리오피스 연계한 자산관리 (10억 이상 자산가 대상)</div></div>', unsafe_allow_html=True)
    
    # 차별화 포인트
    st.markdown('<div style="background: rgba(0, 212, 170, 0.08); padding: 16px; border-radius: 8px; border: 2px solid #00d4aa; margin-top: 16px;"><div style="font-size: 14px; font-weight: 700; color: #00d4aa; margin-bottom: 10px;">💎 차별화 포인트</div><div style="font-size: 13px; color: #cbd5e1; line-height: 1.8;">저는 단순 보험 판매가 아닌, <b style="color: #00d4aa;">데이터 기반 시뮬레이션 도구</b>를 활용하여 "지금 이 상품에 가입하면 10년 후 얼마나 절세되는가"를 실시간으로 보여드립니다.<br><br>XAI, IoT, Streamlit 등 최신 기술로 <b>고객이 스스로 납득</b>할 수 있는 컨설팅을 제공합니다.</div></div>', unsafe_allow_html=True)
    
    # 다음 단계
    st.markdown('<div style="background: #141e2b; padding: 16px; border-radius: 8px; border: 1px solid #1e3a5f; margin-top: 16px;"><div style="font-size: 14px; font-weight: 700; color: #f5a623; margin-bottom: 10px;">📞 다음 단계 — 무료 심층 상담 신청</div><div style="font-size: 12px; color: #c8d8e4; line-height: 1.7; margin-bottom: 12px;">위 분석 결과를 바탕으로 <b style="color: #00d4aa;">귀사 맞춤형 절세 시뮬레이션</b>을 준비해 드리겠습니다.</div><div style="background: rgba(245, 166, 35, 0.05); padding: 12px; border-radius: 6px; border-left: 3px solid #f5a623; margin-bottom: 12px;"><div style="font-size: 11px; font-weight: 700; color: #f5a623; margin-bottom: 6px;">📋 상담 신청 시 제공되는 자료</div><div style="font-size: 11px; color: #c8d8e4; line-height: 1.6;">• 가업승계 시나리오별 세금 비교표 (증여 vs 상속)<br>• 법인보험 가입 시 절세 효과 계산서<br>• 퇴직연금 최적화 방안<br>• ROI 예측 대시보드 (5년/10년 단위)</div></div><div style="background: rgba(0, 212, 170, 0.08); padding: 10px; border-radius: 6px; border: 1px solid #00d4aa; text-align: center;"><span style="font-size: 12px; color: #00d4aa; font-weight: 700;">📧 io7hub@naver.com | ☎ 010-2610-5194</span></div></div>', unsafe_allow_html=True)
    
    st.markdown(f'<div style="text-align: right; color: #64748b; font-size: 10px; margin-top: 16px;">🤖 AI 컨설턴트 분석 완료 | {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</div>', unsafe_allow_html=True)
...
def main():
    ...
    <div class="gfc-header">
      <div class="gfc-live"><div class="gfc-live-dot"></div><span>삼성생명 GFC · 법인 리스크 사전 진단표 v2.0</span></div>
    ...
    with tab_diag:
        ...
        with st.expander("⚙️  Ⅰ. 기본 정보 (Fact Check)", expanded=True):
            ...
            revenue = c6.selectbox("연 매출 규모", 
                                  ...
                                  key="sel_rev")
            c7, c8 = st.columns(2)
            ceo_age = c7.selectbox("대표자 연령대", 
                                  ["─ 선택 ─", "40대 이하", "50대", "60대 이상"], 
                                  key="sel_age")
            ceo_share = c8.selectbox("대표자 지분율", 
                                    ["─ 선택 ─", "50% 미만", "50~80%", "80% 이상"], 
                                    key="sel_shr")
    ...
    with tab_dash:
        scores = st.session_state.get("scores", calc_scores(
            ["아니오"] * 6, ["아니오"] * 6, 
            [AWARENESS[i][1][-1] for i in range(3)], 
            [SCENARIOS[i][1][0] for i in range(3)]
        ))
    ...
    with tab_consult:
        ...
        if not scores:
            ...
            tab_consultant(info, scores, priority, sol_shown)
    ...
    with tab_script:
        ...
        if not scores:
            ...
            <div class="gfc-script">
              ...
              <div class="sc-intro">
                안녕하세요, {info.get('ceo','대표자') or '대표자'} 대표님. 삼성생명 GFC 기업재무컨설팅 <strong>{(info.get('company','') or '○○(주)').replace('(주)','').replace('(유)','').replace('㈜','')}</strong> 담당 컨설턴트 Ken입니다.<br><br>
                오늘 우리회사의 법인 리스크 사전 진단을 완료했는데, 종합 리스크율이 <strong style="color:{clr}">{total_pct:.0f}%({lbl})</strong> 수준으로 나왔습니다.<br>
            ...
            st.markdown(f"""
              ...
              <div class="sc-close">
                오늘 진단 결과를 기반으로, 우리기업에 맞는 <strong>종합 법인 재무 컨설팅 제안서</strong>를 별도로 작성하여 드리겠습니다.<br><br>
